...
本專題的致動器使用 **SG90 伺服馬達**，其 PWM 可用範圍與「中立點」
在不同顆伺服之間可能存在個體差異。
若未進行校正，容易產生以下問題：

- 平台無法回到真正水平（中立偏移）
- 伺服撞到極限，導致抖動、過熱或卡死（Physical Layer 不穩定）
- 相同控制輸入產生不同輸出動作，造成控制結果不可重現

因此，本系統將 **「校正（Calibration）」視為 IoT 系統可重現性的必要流程**，
而非單純的機械調整。

#### 校正工具：`calibrate_endpoints.py`

校正程式用來找出每顆伺服的安全與有效控制範圍，包括：

- `pwm_min`：安全最小 PWM 值  
- `pwm_max`：安全最大 PWM 值  
- `pwm_center`：真正的水平／中立位置  

校正完成後，校正結果會寫回 `config.py`，
作為後續所有控制邏輯的 **單一真實來源（Single Source of Truth, SSOT）**，
確保不同模組與不同執行環境下的行為一致。
...
- `servo_test_one.py`  
  用於確認單顆伺服與供電系統是否能正常運作

- `servo_test_ch2.py`  
  用於確認第二通道或另一顆伺服在相同條件下能一致運作

> **IoT 工程觀點：**  
> 若 Physical / Actuation Layer 的行為無法被穩定重現，  
> 上層的 Web 控制、事件判斷與回饋機制將無法被正確驗證。
