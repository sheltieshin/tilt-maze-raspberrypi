...
def gpio_init():
    ...
    # LED / BUZZER (使用 config 設定，和你測試程式一致)
...
def detect_ir_polarity():
    ...
    # idle==1 => likely active-low trigger (0)
    # idle==0 => likely active-high trigger (1)
    return (idle == 0)
...
def beep(duration=1.0):
    """
    GOAL 蜂鳴器提示：採用你的測試程式同款寫法（True=叫）。
    加鎖避免 /api/center 或其他地方同時把 buzzer 拉 LOW。
    """
    global _beeping
    with buzzer_lock:
        _beeping = True
...
def celebrate():
    """
    IR 過關的 LED + beep pattern
    同樣加鎖，避免跟 GOAL beep 打架。
    """
...
def api_center():
    """
    前端按 Center 或其他流程會打這支。
    注意：不要在「正在 beep」或「GOAL 已觸發」時硬把 buzzer 拉 LOW，
    以免把蜂鳴器聲音掐掉。
    """
    ensure_gpio()
